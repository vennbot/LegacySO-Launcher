...
class Modal {
  ...
  static showReInstall( componentName, componentCode ) {
    Modal.getIPC().sendModal(
      'LegacySO Client',  // Use a fixed title instead of componentName
      locale.current.MODAL_REINSTALL_DESCR_X +
        ' <strong>LegacySO Client</strong>?',
  ...
  static showFailedInstall( componentName, errorMessage ) {
    // Debug logging
    console.log('showFailedInstall called with:', {
        componentName,
        errorMessage
    });

    // Force a message if none provided
    const displayMessage = errorMessage || 'Installation failed - Please try again';

    console.log('Displaying message:', displayMessage);

    Modal.getIPC().sendErrorModal(
        'Installation Failed',
        `${componentName}: ${displayMessage}`,
        'OK'
    );
  }
  ...
  static async showChooseDirectory( componentName, window ) {
    ...
    /**
     * Shows an enhanced directory picker with drive selection and space information.
     *
     * @param {string} componentName The visual name of the Component.
     * @param {Electron.BrowserWindow} window The window to show the picker in.
     *
     * @returns {Promise<string>} The chosen path.
     */
    static async showEnhancedDirectoryPicker(componentName, window) {
        const { getAvailableDrives, checkDiskSpace } = require('./utils');

        try {
            // Get available drives
            const drives = await getAvailableDrives();

            // If only one drive or user prefers simple picker, fall back to standard dialog
            if (drives.length <= 1) {
                return Modal.showChooseDirectory(componentName, window);
            }

            // Create drive selection dialog
            const driveOptions = drives.map(drive => ({
                label: `${drive.label} (${drive.letter}) - ${drive.freeSpaceGB}GB free`,
                value: drive.path,
                drive: drive
            }));

            // Show custom drive selection modal
            const selectedDrive = await Modal.showDriveSelectionModal(driveOptions, componentName, window);

            if (!selectedDrive) {
                return []; // User cancelled
            }

            // After drive selection, show directory picker within that drive
            const response = await require('electron')
                .dialog.showOpenDialog(window,
                    {
                        properties: ['openDirectory'],
                        title: `Choose installation folder on ${selectedDrive.drive.label}`,
                        defaultPath: selectedDrive.value
                    }
                );
            ...
            console.error('Error in enhanced directory picker:', error);
    ...
    /**
     * Shows a drive selection modal.
     *
     * @param {Array} driveOptions Array of drive options.
     * @param {string} componentName The component name.
     * @param {Electron.BrowserWindow} window The window.
     *
     * @returns {Promise<Object|null>} Selected drive option or null if canceled.
     */
    static async showDriveSelectionModal(driveOptions, componentName, window) {
        return new Promise((resolve) => {
            const { dialog } = require('electron');

            // Create a simple selection dialog using message box
            const buttons = driveOptions.map(option => option.label);
            buttons.push('Cancel');

            dialog.showMessageBox(window, {
                type: 'question',
                title: `Select Drive for ${componentName}`,
                message: `Choose which drive to install ${componentName} on:`,
                detail: 'Select a drive with sufficient free space for the installation.',
                buttons: buttons,
                defaultId: 0,
                ...
                if (result.response === buttons.length - 1) {
                    ...
                    resolve(driveOptions[result.response]);
                }
            }).catch(() => {
                resolve(null);
            });
  ...
  // Uninstall selection modal has been removed
}
