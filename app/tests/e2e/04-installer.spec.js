...
test.describe( 'installer', () => {
  ...
  test( 'installs Remesh Package', async () => {
    ...
    const dirPath = process.platform === 'win32' ?
      T.getInstallDir() + '/LegacySO Game/FreeSO/Content/MeshReplace' :
      T.getInstallDir() + '/LegacySO/Content/MeshReplace';
...
async function killGame() {
  // Kill LegacySO.exe on Windows and macOS
  if ( process.platform === 'win32' ) {
    try {
      console.info( 'killing any running instances of LegacySO.exe...' );
      console.info( await exec( 'taskkill /F /IM legacyso.exe' ) );
    } catch ( err ) {
      console.error( 'error killing LegacySO:', err );
    }
  } else if ( process.platform === 'darwin' ) {
    try {
      console.info( 'killing any running instances of LegacySO on macOS...' );
      // 'pgrep' finds the process ID of a running program
      const { stdout: pid } = await exec( 'pgrep -f legacyso.command' );
